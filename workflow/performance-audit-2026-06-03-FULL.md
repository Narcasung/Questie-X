...
| P2 l10n per-call alloc | open | **in progress** | 1016 `l10n(...)` call sites; `Localization/l10n.lua` now has a no-arg fast path and locale cache, but formatted calls still allocate `local args = {...}`. |
| PP1 `IsDoable` 12+ hash lookups | open | **still open** | Not refactored. |
| P5 tooltip party-map cache | open | **still open** | `QuestiePlayer:GetPartyMemberByName` still iterates 1-40 per call (4 callers). |
...
| P7 `CalculateAndDrawAll` debounce | open | **still open** | 2 immediate calls + 3 `Delay(0.3,…)` calls in `QuestieOptionsGeneral.lua:337/357/376/395/418`. |
...
- `Localization/l10n.lua`
  ...
  - The formatted-string path still uses the existing vararg table when arguments are actually supplied.
- `Tests/AuditFindings_spec.lua`
...
path and a cache for repeated literal lookups. The formatted-string path still
allocates `local args = {...}`, so `P2` is improved but not fully closed.
