...
function QuestieWotlkObjectFixes:LoadFactionFixes()
    ...
    -- Blood of Heroes (ID 176213): faction-neutral object with spawns across Plaguelands.
    -- Placed here so it enters objectDataOverrides, which is the path GetObject() actually reads.
    -- The Load() path writes to objectData (the binary DB companion) which GetObject does NOT check directly.
    local objectFixesShared = {
        [176213] = {
            [objectKeys.spawns] = {
    ...
    -- Merge shared (faction-neutral) fixes into both faction tables
    for id, data in pairs(objectFixesShared) do
        objectFixesHorde[id] = data
        objectFixesAlliance[id] = data
    end
