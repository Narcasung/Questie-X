...
- **[Phase 3 - l10n Bounded Formatter]** Reworked formatted translation calls in `Localization/l10n.lua` to use a bounded `select()`-based formatter for the common small-argument cases, with a table fallback only for unusually long arg lists. This removes the hot `{...}` allocation path from the typical `l10n("...", ...)` calls.
- **[Phase 3 - IsDoable Batch Query]** Reworked `QuestieDB.IsDoable` to pull all of its eligibility fields through one batch `QueryQuest(...)` read instead of issuing a dozen single-field lookups per quest. This keeps the hot eligibility path logically identical while cutting repeated DB handle calls.
- **[Phase 3 - Profile Alias Cache]** Cached `Questie.db.profile` in the hottest tooltip/map paths (`MapIconTooltip`, `QuestieMap`, `Tooltip`) so repeated UI toggles and render checks no longer walk the full `Questie -> db -> profile` chain on every branch.
- **[Phase 3 - GetTime Hoist]** Hoisted repeated `GetTime()` checks into local `now` variables in `MapIconTooltip:Show`, `QuestieLearnerComms:IsSenderTrusted`, and `QuestieInit:Stage3` so each hot path samples the clock once per branch instead of multiple times.
- **[Phase 3 - NPC Name Lookup Cache]** Replaced the rare `QuestieQuestPrivates` name-fallback scan over the full NPC table with a cached lowercase name index so custom-server fallback lookups stay cheap even when the objective data has to be inferred by name.
- **[Phase 3 - Validate Cache Closure Cleanup]** Replaced the per-quest anonymous `pcall(function() ...)` closure in `QuestieValidateGameCache` with a named helper so the login validation loop no longer allocates a fresh closure for each quest.
...
- **[Phase 3 - Available Quest Batch Draw]** Removed the per-quest coroutine spawn from `AvailableQuests._DrawAvailableQuest` so the available-quest sweep now reuses the existing batch thread instead of creating one coroutine per quest.
